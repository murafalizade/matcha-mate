...
export default function HomeScreen() {
                        ...
                        [
                            {
                                ...
                                onPress: () =>
                                    router.push(
                                        `/(authorized)/(chats)/message?id=${result.chatSession!.id}`,
                                    ),
                            },
                        ],
                    );
    ...
    return (
        <View className="flex-1 bg-gray-100 mt-[64px]">
            {error && <Text className="text-center text-red-500 py-2 bg-red-50">{error}</Text>}
            <FlatList
                data={profiles}
                renderItem={({ item }) => <RenderProfile item={item} onLike={handleLike} />}
